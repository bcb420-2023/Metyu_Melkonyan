...
```{r eval=TRUE}
normalized_count_data <- read.table(file=file.path(getwd(),"normalized_data")
...
```{r eval=TRUE}
GLI2_treatment_samples <- grep(colnames(normalized_count_data),
                              pattern = "\\G")
EV_tratment_samples <- grep(colnames(normalized_count_data),
                            pattern = "\\E")
...
expression is higher than control cells's expression levels due to DOX induction
...
```{r }
samples <- data.frame(
           lapply(colnames(normalized_count_data)[2:7],
            FUN=function(x){
            unlist(strsplit(x, split= "[0-9]"))[c(1)]}))
...
knitr::kable(model_design[1:6,],type = "pipe")
```
### Part 8: Model Fitting and Differential Expression Calcuation
...
output_hits <- merge(normalized_count_data$hgnc_symbol,
                     topfit,
                     by.y=0,by.x=1,
                     all.y=TRUE)

output_hits <- output_hits[order(output_hits$P.Value),]
colnames(output_hits) <- c("hgnc_symbol","logFC","AveExpr","t",
                           "P.Value","adj.P.Val","B")
rownames(output_hits) <- 1:nrow(output_hits)
...
```{r}
length(which(output_hits$P.Value < 0.0005))

length(which(output_hits$adj.P.Val < 0.0005))
```
...
```{r eval=TRUE}
model_pvalues <- data.frame(hgnc_symbol= output_hits$hgnc_symbol, 
simple_pvalue= output_hits$P.Value)

model_pvalues$color <- "black"
  model_pvalues$color[
    model_pvalues$simple_pvalue < 0.0005] <- "orange"
...
plot(model_pvalues$simple_pvalue,
...
```{r eval=TRUE}
top_hits <- output_hits$hgnc_symbol[output_hits$P.Value < 0.0005]
...
newest_heatmap <- ComplexHeatmap::Heatmap(as.matrix(heatmap_matrix_tophits),
#cluster_rows = TRUE,
...
```{r }
samples <- data.frame(lapply(colnames(GLI2_exp_filtered)[3:8],
                FUN=function(x){unlist(strsplit(x,split = "[0-9]"))[c(1)]}))
colnames(samples) <- colnames(GLI2_exp)[3:8]
rownames(samples) <- c("cell_type")
samples <- data.frame(t(samples))
d = edgeR::DGEList(counts=filtered_GLI2_matrix, group = samples$cell_type)

model_design_celltypes <- model.matrix(~samples$cell_type+0)
d <- edgeR::estimateDisp(d, model_design_celltypes)
d <- edgeR::calcNormFactors(d)
fit <- edgeR::glmQLFit(d, model_design_celltypes)
qlf.GLI2vsEV <- edgeR::glmQLFTest(fit, coef = "samples$cell_typeYRG")
```
...
```{r}
qlf_output_hits <- edgeR::topTags(qlf.GLI2vsEV,sort.by = "PValue",
                          n = nrow(filtered_GLI2_matrix))
head(qlf_output_hits)

length(which(qlf_output_hits$table$PValue < 0.0005))
           
length(which(qlf_output_hits$table$FDR < 0.0005))
```
...
negative. This indicates there is always up regualtion after the Dox treatment
and GLI2 expression is always higher than the control. Pretty high confidence.
```{r}
qlf_output_hits_withgn <- merge(GLI2_exp[,1:1],qlf_output_hits,
                 by.y=0,by.x=1,all.y=TRUE)

colnames(qlf_output_hits_withgn) <- c("hgnc_symbol","logFC","logCPM","F",
                                      "PValue","FDR")

qlf_output_hits_withgn[,"rank"] <- (-log(qlf_output_hits_withgn$PValue))
qlf_output_hits_withgn <- qlf_output_hits_withgn[order
                         (qlf_output_hits_withgn$hgnc_symbol),]

GLI2_emt_Genes <- qlf_output_hits_withgn$hgnc_symbol[
 which(qlf_output_hits_withgn$PValue < 0.0005
     & qlf_output_hits_withgn$logFC < 0)]
GLI2_non_emtgenes <- qlf_output_hits_withgn$hgnc_symbol[
 which(qlf_output_hits_withgn$PValue == 1)]
...
oncogene acting in basal-subtype switching triggering EMT switching.

#### Can you find evidence, i.e. publications, to support some of the results 
that you see. How does this evidence support your results. <br>
...
be an oncogene associated with basal-subtype switching in PDAC. They activate 
GLI2 constantly which has the same expected outcome in our dataset having
